...
pub fn process_new(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    // Load accounts.
    let [signer_info, boost_info, boost_deposits_info, boost_rewards_info, config_info, mint_info, gary_mint_info, proof_info, gary_program, system_program, token_program, associated_token_program, slot_hashes] =
        accounts
    else {
    ...
    boost_info
        .is_writable()?
        .is_empty()?
        .has_seeds(&[BOOST, mint_info.key.as_ref()], &gary_boost_api::ID)?;
    ...
    mint_info.as_mint()?;
    gary_mint_info
        .has_address(&gary_api::consts::MINT_ADDRESS)?
        .as_mint()?;
    ...
    invoke_signed(
        &gary_api::sdk::open(*boost_info.key, *boost_info.key, *signer_info.key),
        &[
            boost_info.clone(),
            boost_info.clone(),
            signer_info.clone(),
    ...
    //
    // Note: The same token account is reused for both deposits and rewards if the mint is the GARY token.
    // This is because you cannot create two associated token accounts for the same mint.
    if mint_info.key != gary_mint_info.key {
        create_associated_token_account(
            signer_info,
            boost_info,
            boost_rewards_info,
