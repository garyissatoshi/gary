...
use gary_api::state::proof_pda;
use gary_pool_api::state::member_pda;
use gary_pool_types::{
...
impl Miner {
    ...
    async fn list_pools(&self, _args: PoolArgs) -> Result<(), Error> {
        ...
        for (pool_address, pool) in pools {
            ...
            if let Ok(proof) = get_proof(&self.rpc_client, proof_address).await {
                point.balance = format!("{} GARY", amount_u64_to_f64(proof.balance));
                point.last_hash_at = format_timestamp(proof.last_hash_at);
                point.lifetime_hashes = proof.total_hashes.to_string();
                point.lifetime_rewards = format!("{} GARY", amount_u64_to_f64(proof.total_rewards));
            }
    ...
    async fn get_pool(&self, pool_url: String) -> Result<(), Error> {
        ...
        data.push(TableData {
            ...
            value: format!("{} GARY", amount_u64_to_f64(proof.balance))
        });
        data.push(TableData {
        ...
        data.push(TableData {
            ...
            value: format!("{} GARY", amount_u64_to_f64(proof.total_rewards)),
        });
        data.push(TableData {
            key: "Miner".to_string(),
        ...
        if let Ok(member) = member {
            ...
            data.push(TableData {
                key: "Balance".to_string(),
                value: format!("{} GARY", utils::amount_u64_to_string(member.balance))
                    .bold()
                    .yellow()
                    .to_string(),
            });
            ...
            if let Ok(member_offchain) = pool.get_pool_member(&self).await {
                ...
                data.push(TableData {
                    ...
                    value: format!("{} GARY", utils::amount_u64_to_string(pending_rewards)),
                });
            }
            data.push(TableData {
                key: "Lifetime rewards".to_string(),
                value: format!("{} GARY", utils::amount_u64_to_string(member.total_balance)),
            });
        }
...
impl Pool {
    pub async fn post_pool_register(&self, miner: &Miner) -> Result<Member, Error> {
        ...
        if let Err(_err) = self.get_pool_member_onchain(miner, pool_pda.address).await {
            ...
            let ix = gary_pool_api::sdk::join(pubkey, pool_pda.address, pubkey);
            let _ = miner
    ...
    pub async fn get_pool_member_onchain(
        ...
        miner: &Miner,
        pool_address: Pubkey,
    ) -> Result<gary_pool_api::state::Member, Error> {
        let (member_pda, _) =
            gary_pool_api::state::member_pda(miner.signer().pubkey(), pool_address);
        let data = miner.rpc_client.get_account_data(&member_pda).await?;
        let pool = gary_pool_api::state::Member::try_from_bytes(data.as_slice())?;
        Ok(*pool)
    }
    ...
    pub async fn _get_staker_onchain(
        ...
        pool_address: Pubkey,
        mint: Pubkey,
    ) -> Result<gary_pool_api::state::Share, Error> {
        let (share_pda, _) =
            gary_pool_api::state::share_pda(miner.signer().pubkey(), pool_address, mint);
        let data = miner.rpc_client.get_account_data(&share_pda).await?;
        let share = gary_pool_api::state::Share::try_from_bytes(data.as_slice())?;
        Ok(*share)
    }
    ...
    pub async fn get_latest_pool_event(
        ...
        authority: Pubkey,
        last_hash_at: i64,
    ) -> Result<gary_pool_types::PoolMemberMiningEvent, Error> {
        ...
        loop {
            ...
            match resp.error_for_status() {
                ...
                Ok(resp) => {
                    if let Ok(event) = resp.json::<gary_pool_types::PoolMemberMiningEvent>().await {
    ...
    pub async fn post_update_balance(&self, miner: &Miner) -> Result<(), Error> {
        ...
        let pool = gary_pool_api::state::Pool::try_from_bytes(data.as_slice())?;
        let pool_authority = pool.authority;

        // build attribute instruction
        let ix = gary_pool_api::sdk::attribute(
            pool_authority,
            *signer_pubkey,
