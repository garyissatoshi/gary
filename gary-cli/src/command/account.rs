...
impl Miner {
    ...
    async fn get_account_data(&self, authority: Pubkey, data: &mut Vec<TableData>) {
        ...
        data.push(TableData {
            ...
            value: format!("{} GARY", token_balance),
        });
        data.push(TableData {
            key: "SOL".to_string(),
        ...
        if let Ok(proof) = proof {
            ...
            data.push(TableData {
                ...
                value: if proof.balance > 0 {
                    format!("{} GARY", amount_u64_to_f64(proof.balance)).bold().yellow().to_string()
                } else {
                    format!("{} GARY", amount_u64_to_f64(proof.balance))
                }
            });
            data.push(TableData {
            ...
            data.push(TableData {
                ...
                value: format!("{} GARY", amount_to_ui_amount(proof.total_rewards, gary_api::consts::TOKEN_DECIMALS)),
            });
            data.push(TableData {
                key: "Miner".to_string(),
    ...
    async fn close(&self, _args: AccountCloseArgs) {
        ...
        if !ask_confirm(
            format!("{} You have {} GARY staked in this account.\nAre you sure you want to {}close this account? [Y/n]",
                "WARNING".yellow(),
                amount_to_ui_amount(proof.balance, gary_api::consts::TOKEN_DECIMALS),
        ...
        let ix = gary_api::sdk::close(signer.pubkey());
