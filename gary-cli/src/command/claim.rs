...
use colored::*;
use gary_api::consts::MINT_ADDRESS;
...
impl Miner {
    ...
    pub async fn claim_from_proof(&self, args: ClaimArgs) {
        ...
        if !ask_confirm(
            format!(
                ...
                format!(
                    "{} GARY",
                    amount_to_ui_amount(amount, gary_api::consts::TOKEN_DECIMALS)
                )
                .bold(),
            )
            .as_str(),
        ...
        // Send and confirm
        ixs.push(gary_api::sdk::claim(pubkey, beneficiary, amount));
    ...
    async fn claim_from_pool(
        ...
        // Send and confirm
        ixs.push(gary_pool_api::sdk::claim(
            self.signer().pubkey(),
            beneficiary,
