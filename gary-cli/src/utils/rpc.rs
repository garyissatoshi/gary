...
use gary_pool_api::state::{Member, Pool};
...
pub async fn get_boost_config(client: &RpcClient) -> gary_boost_api::state::Config {
    let data = client
        .get_account_data(&gary_boost_api::state::config_pda().0)
        .await
        .expect("Failed to get config account");
    *gary_boost_api::state::Config::try_from_bytes(&data).expect("Failed to parse config account")
}
...
pub async fn get_boosts(client: &RpcClient) -> Result<Vec<(Pubkey, Boost)>, anyhow::Error> {
    get_program_accounts::<Boost>(client, gary_boost_api::ID, vec![]).await
}

pub async fn get_pools(client: &RpcClient) -> Result<Vec<(Pubkey, Pool)>, anyhow::Error> {
    get_program_accounts::<Pool>(client, gary_pool_api::ID, vec![]).await
}
...
pub async fn get_boost_stake_accounts(
    ...
    get_program_accounts::<Stake>(rpc_client, gary_boost_api::ID, vec![filter]).await
}
