...
use crate::params::{EQUIHASH_K, EQUIHASH_N};
use crate::{Error, HashValue, SolutionItem};
use core::cmp;
...
impl Solution {
    ...
    pub const NUM_BYTES: usize = Self::NUM_ITEMS * size_of::<SolutionItem>();

    /// Size of each [`SolutionItem`], in bytes
    const ITEM_SIZE: usize = size_of::<SolutionItem>();
    ...
    ///
    /// Used by the solver.
    pub fn sort_from_array(mut items: SolutionItemArray) -> Self {
    ...
    pub fn try_from_bytes(bytes: &SolutionByteArray) -> Result<Self, Error> {
        let mut array: SolutionItemArray = [0; Self::NUM_ITEMS];
    ...
    pub fn to_bytes(&self) -> SolutionByteArray {
        let mut result: SolutionByteArray = [0; Self::NUM_BYTES];
        for i in 0..Self::NUM_ITEMS {
...
pub fn check_all_tree_sums(func: &HashX, solution: &Solution) -> Result<(), Error> {
