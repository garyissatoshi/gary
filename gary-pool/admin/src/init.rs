use gary_pool_api::state::pool_pda;
...
pub async fn init(
    ...
    if pool_data.is_err() {
        ...
        let launch_ix = gary_pool_api::sdk::launch(pool_authority, pool_authority, pool_url)?;
        let mut tx =
    ...
    let (member_address, _) = gary_pool_api::state::member_pda(pool_authority, pool_address);
    ...
    if member_data.is_err() {
        let cu_budget = ComputeBudgetInstruction::set_compute_unit_limit(1_000_000);
        let cu_price = ComputeBudgetInstruction::set_compute_unit_price(1_000_000);
        let join_ix = gary_pool_api::sdk::join(pool_authority, pool_address, pool_authority);
        let mut tx =
