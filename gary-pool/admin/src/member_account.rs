use gary_pool_api::state::Member;
...
pub async fn member_account_lookup(
    ...
    let pubkey = pubkey?;
    let (pool_pda, _) = gary_pool_api::state::pool_pda(keypair.pubkey());
    let (member_pda, _) = gary_pool_api::state::member_pda(pubkey, pool_pda);
    println!("member address: {:?}", member_pda);
    let data = rpc_client.get_account_data(&member_pda).await?;
    let member = Member::try_from_bytes(data.as_slice())?;
    println!("{:?}", member);
...
pub async fn member_account_gpa(
    ...
    let vec = rpc_client.get_program_accounts(&gary_pool_api::ID).await?;
    let vec: Vec<_> = vec
        .into_iter()
