...
pub fn process_unstake(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse args.
    let args = gary_pool_api::instruction::Unstake::try_from_bytes(data)?;
    let amount = u64::from_le_bytes(args.amount);

    // Load accounts.
    let [signer_info, boost_info, boost_tokens_info, mint_info, member_info, pool_info, pool_tokens_info, recipient_tokens_info, share_info, stake_info, token_program, gary_boost_program] =
        accounts
    else {
    ...
    mint_info.as_mint()?;
    member_info
        .as_account::<Member>(&gary_pool_api::ID)?
        .assert(|m| m.authority == *signer_info.key)?
        .assert(|m| m.pool == *pool_info.key)?;
    let pool = pool_info.as_account::<Pool>(&gary_pool_api::ID)?;
    ...
    let share = share_info
        .as_account_mut::<Share>(&gary_pool_api::ID)?
        .assert_mut(|s| s.authority == *signer_info.key)?
        .assert_mut(|s| s.pool == *pool_info.key)?
        .assert_mut(|s| s.mint == *mint_info.key)?;
    token_program.is_program(&spl_token::ID)?;
    gary_boost_program.is_program(&LEGACY_BOOST_PROGRAM_ID)?;
