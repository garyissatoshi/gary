...
pub fn pool_proof_pda(pool: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[gary_api::consts::PROOF, pool.as_ref()], &gary_api::id())
}
...
pub fn pool_stake_pda(pool: Pubkey, mint: Pubkey) -> (Pubkey, u8) {
    let boost_pda = gary_boost_api::state::boost_pda(mint);
    gary_boost_api::state::stake_pda(pool, boost_pda.0)
}
...
pub fn legacy_boost_pda(mint: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(
        &[gary_boost_api::consts::BOOST, mint.as_ref()],
