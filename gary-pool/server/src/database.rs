...
pub async fn stream_members_attribution(
    ...
    while let Some(row) = stream.try_next().await? {
        ...
        // build instruction
        let ix = gary_pool_api::sdk::attribute(signer, member_authority, total_balance as u64);
        ix_buffer.push(ix);
...
pub async fn write_new_member(
    conn: &Object,
    member: &gary_pool_api::state::Member,
    approved: bool,
) -> Result<gary_pool_types::Member, Error> {
    let member = gary_pool_types::Member {
...
pub async fn read_member(conn: &Object, address: &String) -> Result<gary_pool_types::Member, Error> {
    let row = conn
        .query_one(
            &format!(
