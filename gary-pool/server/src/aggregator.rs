...
impl Aggregator {
    ...
    fn insert(&mut self, contribution: &mut Contribution) -> Result<(), Error> {
        ...
        if !gary_drillx::is_valid_digest(&challenge.challenge, &solution.n, &solution.d) {
    ...
    async fn submit_and_reset(&mut self, operator: &Operator) -> Result<(), Error> {
        ...
        let (pool_address, _) = gary_pool_api::state::pool_pda(*authority);
        let (pool_proof_address, _) = gary_pool_api::state::pool_proof_pda(pool_address);
        let bus = self.find_bus(operator).await?;

        // Get boost accounts
        let mut boost_accounts: Option<[Pubkey; 3]> = None;
        let boost_config_address = gary_boost_api::state::config_pda().0;
        let rpc_client = &operator.rpc_client;
        let accounts = rpc_client.get_account(&boost_config_address).await?;
        if let Ok(boost_config) = gary_boost_api::state::Config::try_from_bytes(&accounts.data) {
            boost_accounts = Some([
        ...
        // build instructions
        let auth_ix = gary_api::sdk::auth(pool_proof_address);
        let submit_ix = gary_pool_api::sdk::submit(
        ...
        let rotate_ix = gary_boost_api::sdk::rotate(operator.keypair.pubkey());
