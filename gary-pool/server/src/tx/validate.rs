...
use gary_pool_api::{instruction::Attribute, prelude::PoolInstruction};
...
pub fn validate_attribution(
    ...
    if attr_program_id.ne(&gary_pool_api::id()) {
        return Err(Error::Internal(
            "first instruction after compute budget and lighthouse instructions must be an gary_pool instruction".to_string(),
        ));
    }
    ...
    let (member_pda, _) = gary_pool_api::state::member_pda(member_authority, pool);
    ...
    let mut gary_pool_end_idx = first_non_allowed_prefix_idx + 1;

    // Check for an spl-associated-token-account instruction that might come before the claim
    if gary_pool_end_idx < n {
        let next_ix = &instructions[gary_pool_end_idx];
    ...
    if gary_pool_end_idx < n {
        let claim_ix = &instructions[gary_pool_end_idx];
        ...
        // If the instruction is from gary_pool, validate it as a claim instruction
        if claim_program_id.eq(&gary_pool_api::id()) {
            ...
            if claim_tag.ne(&PoolInstruction::Claim) {
                return Err(Error::Internal(
                    "gary_pool instruction after attribution must be a claim instruction"
                        .to_string(),
                ));
            }
