...
use gary_api::state::{Config, Proof};
...
impl Operator {
    ...
    pub async fn get_pool(&self) -> Result<Pool, Error> {
        ...
        let rpc_client = &self.rpc_client;
        let (pool_pda, _) = gary_pool_api::state::pool_pda(authority);
    ...
    pub async fn get_member_onchain(&self, member_authority: &Pubkey) -> Result<Member, Error> {
        ...
        let rpc_client = &self.rpc_client;
        let (pool_pda, _) = gary_pool_api::state::pool_pda(authority);
        let (member_pda, _) = gary_pool_api::state::member_pda(*member_authority, pool_pda);
    ...
    pub async fn get_member_db(
    ...
    ) -> Result<gary_pool_types::Member, Error> {
        ...
        let (pool_pda, _) = gary_pool_api::state::pool_pda(pool_authority);
        let (member_pda, _) = gary_pool_api::state::member_pda(member_authority, pool_pda);
    ...
    async fn get_config(&self) -> Result<Config, Error> {
        let config_pda = gary_api::consts::CONFIG_ADDRESS;
        let rpc_client = &self.rpc_client;
