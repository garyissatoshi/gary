...
pub async fn address(operator: web::Data<Operator>) -> impl Responder {
    let operator = operator.as_ref();
    let (pool_pda, bump) = gary_pool_api::state::pool_pda(operator.keypair.pubkey());
...
async fn update_balance_onchain(
    ...
    let (pool_address, _) = gary_pool_api::state::pool_pda(keypair.pubkey());
    ...
    let (member_address, _) = gary_pool_api::state::member_pda(member_authority, pool_address);
